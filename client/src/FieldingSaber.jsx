...
function FieldingSaber({ searchPlayer, setSearchPlayer }) {
  ...
  const fielderImage = searchPlayer
    ? `https://img.mlbstatic.com/mlb-photos/image/upload/v1/people/665742/headshot/silo/current`
  ...
  useEffect(() => {
    ...
    fetch(
      `https://baseballsavant.mlb.com/player-services/range?playerId=665742&season=${selectedSeason}&playerType=fielder`
    )
      .then((resp) => resp.json())
      .then((data) => {
        setFeildingData(data);
      });
  ...
  return (
    <div className="WrappersaberField">
      <div className="topLineWrapper">
        ...
        <div>
          <Search
            ...
            setFetchedPlayers={setFetchedPlayers}
          />
        </div>
      </div>
      ...
      <div>
        <FieldingCard
          fielderImage={fielderImage}
          feildingData={feildingData}
