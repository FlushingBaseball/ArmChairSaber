...
function NewPredictionGroup({ game, user, key, setPredictedWinner }) {
          ...
          if (!response.ok) {
            ...
            fetch(`/api/users/${user.id}`, {
              ...
              .then((resp) => {
                if (!resp.ok) {
                  throw new Error(
                    "Failed to update user total num predictions"
                  );
                }
                return resp.json();
              })
  ...
  return (
    <div
      className={`predictiveinfoGroup ${
        submitNotSignedIn ? "invalidEntry" : null
      }`}
    >
...
export default NewPredictionGroup;
